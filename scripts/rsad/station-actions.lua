...
function RSAD_Actions.exec_decouple(train, controller, scope, params)
  ...
  if not decouple_at then
    log({ "", "Could not decouple " ..
    params.wagon_count ..
    " carriage offset from train with " ..
    #carriages .. " carriages from rear" .. ((params.ignore_locomotive and " locomotive") or ".") })
    return -1
  end
...
function RSAD_Actions.exec_align(train, controller, scope, alignment)
  if not RSAD_Actions.resolve_runtime_params(train, controller, scope, table) then return -1 end
  if not alignment.continue_no_alignment and not alignment.ignore_locomotives and (#train.carriages <= alignment.offset_num) then return
    train.id end
  ...
  local seek_dir = (alignment.offset_from == defines.rail_direction.back and defines.rail_direction.front) or
  defines.rail_direction.back
  while alignment.ignore_locomotives and carriage and carriage.type == "locomotive" do
    carriage = carriage.get_connected_rolling_stock((carriage.is_headed_to_trains_front and seek_dir) or
    ((seek_dir == defines.rail_direction.front and defines.rail_direction.back) or defines.rail_direction.front))
  end
  local next_carriage = carriage
  for i = 1, alignment.offset_num, 1 do
    carriage = next_carriage
    next_carriage = carriage and
    carriage.get_connected_rolling_stock((carriage.is_headed_to_trains_front and seek_dir) or
    ((seek_dir == defines.rail_direction.front and defines.rail_direction.back) or defines.rail_direction.front))
...
local runtime_param_handlers = {
  ...
  [RuntimeParamType.starting.train.arrival_direction] = function(train, controller, station)
    ...
    if not entity or not entity.valid then
      ...
      local moved = false
      if not next_rail.move_forward(defines.rail_connection_direction.straight) then if not next_rail.move_forward(defines.rail_connection_direction.right) then moved =
          next_rail.move_forward(defines.rail_connection_direction.left) else moved = true end else moved = true end
  ...
  [RuntimeParamType.starting.train.reversed_direction] = function(train, controller, station)
    ...
    return (select(2, get_front_stock(train, entity)) == defines.rail_direction.front and defines.rail_direction.back) or
...
function RSAD_Actions.resolve_runtime_params(train, controller, scope, table)
  for name, param in pairs(table) do
    if type(param) == "table" and param.runtime then
      ...
      if value == nil then
        log({ "", "Could not resolve runtime param type [" ..
        param.variable ..
        "] for train [" ..
        ((train and train.id) or "nil") ..
        "] and station [" .. ((scope.station and scope.station.unit_number) or "none") .. "]" })
        return false
      end
      table[name] = value
    end
...
function RSAD_Actions.start_actionset_execution(train, set, controller, station)
  ...
  for _, action in pairs(set.scope.actions) do
    ...
    if type(result) == "number" then
      if result < 0 then
        log({ "", "Error occured when executing action set at train [" .. (train and train.valid and train.id) or
        "unknown" .. "]" })
        return false, train
      end

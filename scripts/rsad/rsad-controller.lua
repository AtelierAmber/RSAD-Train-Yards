...
function rsad_controller.register_events(self)
  ...
  events.register_break("name", names.entities.rsad_station,
    function(entity) return self:__on_station_destroyed(entity) end)
  ...
  events.register_train_handler(defines.events.on_train_changed_state,
    function(data) self:__on_train_state_change(data.train, data.old_state) end)
...
function rsad_controller.__on_arrive_at_station(self, station, train, old_state)
  ...
  local action_set = station.arrival_actions[(shunting_info and shunting_info.current_stage)] or
  station.arrival_actions[rsad_shunting_stage.unspecified]
  if action_set then

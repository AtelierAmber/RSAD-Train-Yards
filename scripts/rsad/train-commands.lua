...
function rsad_controller.couple_direction(self, train, direction)
  ...
  local connecting_stock = direction == (defines.rail_direction.front and train.carriages[0]) or
  train.carriages[#train.carriages]
  local connecting_id = connecting_stock and connecting_stock.train and connecting_stock.train.id
  local connect_dir = connecting_stock.get_connected_rolling_stock(defines.rail_direction.front) and
  ...
  if not train then
    log({ "", "Train is nil after coupling train" })
    return -1
  end
...
function rsad_controller.decouple_at(self, train, at, direction, park_self)
  ...
  local old_interrupts = old_lua_schedule.get_interrupts()
  local old_current = old_lua_schedule.current
  local other = at.get_connected_rolling_stock((at.is_headed_to_trains_front and direction) or
  ((direction == defines.rail_direction.front and defines.rail_direction.back) or defines.rail_direction.front))
  if not at.disconnect_rolling_stock((at.is_headed_to_trains_front and direction) or ((direction == defines.rail_direction.front and defines.rail_direction.back) or defines.rail_direction.front)) then
    log({ "", "Failed to decouple train [" .. train.id .. "]" })
    return -1
  end
  train = at.train
  if not train then
    log({ "", "Train is nil after decoupling train" })
    return -1
  end
  local new_id = train.id
...
function rsad_controller.move_train(self, train, distance, direction)
  ...
  if not segment_end.move_forward(defines.rail_connection_direction.straight) then if not segment_end.move_forward(defines.rail_connection_direction.right) then
      segment_end.move_forward(defines.rail_connection_direction.left) end end
  segment_end.move_to_segment_end()

...
local function generate_target_record(target_station, reversed)
  ...
  local reversed_rail_direction = target_entity.connected_rail_direction == defines.rail_direction.back and
  defines.rail_direction.front or defines.rail_direction.back
...
local function remove_wagon_schedule(yard, removal_station)
  ...
  if not rail_end.move_forward(defines.rail_connection_direction.straight) then if not rail_end.move_forward(defines.rail_connection_direction.right) then
      rail_end.move_forward(defines.rail_connection_direction.left) end end
...
function scheduler.return_shunter(self, train, yard)
  ...
  for _, depot in pairs(yard[rsad_station_type.shunting_depot]) do
    ...
    if not depot.parked_train and depot.incoming <= 0 then
      ...
      ---target_entity must be not nil if target_rail is not nil
  ...
  local path = game.train_manager.request_train_path({ type = "any-goal-accessible", starts = free_depot_starts, goals = { train.front_end, train.back_end }, train =
  train, in_chain_signal_section = false })

  if path.found_path then
    return_depot_start = free_depot_starts[path.start_index]
    local reversed_rail_direction = return_depot_start.direction == defines.rail_direction.back and
    defines.rail_direction.front or defines.rail_direction.back
    local records = {
      [1] = {
...
local function calculate_brake_force(traveled, brake_force, speed, destination)
  ...
  local brake_comp = math.min(1, (dist_to_start / brake_distance)) *
  brake_force                                                                      --Distributes an adjustment factor across the braking distance, capped to brake_force (brake_force x2)
...
function scheduler.process_script_movement(self)
  ...
  for id, data in pairs(self.scripted_trains) do
    if not data.train or not data.train.valid then
      killed[id] = id
      goto continue
    end
    if data.stopping and not data.train.manual_mode then
      killed[id] = id
      goto continue
    end                                                                                        --Train was manually changed by player and is no longer controlled by script

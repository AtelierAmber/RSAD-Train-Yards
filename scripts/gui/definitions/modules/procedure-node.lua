...
local function build_node(name, class_name, content)
    ...
    builder.frame("node_frame", "train_schedule_partially_fullfilled_condition_frame", true, {size = {260, 260}, padding = 3}){
      builder.vflow(nil, nil, {vertical_spacing = 0}){
        builder.frame(nil, "deep_frame_in_shallow_frame_for_description", false, {horizontally_stretchable = true}){
          ...
          builder.text_box("node_name", name, false, "rsad_editable_label"),
...
local function make_param(name, interface, prename_args, postname_args, prename_args_count)
  ...
  if prename_args then
    if postname_args then
      table.insert(built, interface(table.unpack(prename_args, 1, prename_args_count or #prename_args), name .. ".interface", table.unpack(postname_args, #postname_args)))
    else
      table.insert(built, interface(table.unpack(prename_args, prename_args_count or #prename_args), name .. ".interface"))
    end
  elseif postname_args then
    table.insert(built, interface(name .. ".interface"), table.unpack(postname_args))
  else
    table.insert(built, interface(name .. ".interface"))
  end
  
  return builder.hflow(name)(built)
end

---@type table<RSAD.Procedure.BuiltInAction, RSAD.GuiBuilder|GuiElemDef[]>
local builtin_action_param_guis = {
  [BuiltInAction.move] = {
    make_param("param_move_type", builder.dropdown),
    make_param("param_distance", builder.slider),
    make_param("param_direction", builder.dropdown),
  }
}
...
local builtin_action_updators = {
  [BuiltInAction.move] = function(action, refs)
    local move_type_gui = refs["param_move_type"]
    local distance_gui = refs["param_distance"]
    local direction_gui = refs["param_direction"]
    if not move_type_gui or not distance_gui or not direction_gui then error("Failed to construct move node!") end

    --- Move Type
    local movetype_interface = move_type_gui["param_move_type.interface"]
    local movetypeitems = {}
    local movetype_index = -1
    for _, t in pairs(MoveType) do
      table.insert(movetypeitems, {"rsad-actions." .. t.name})
      if t.name == action.params.move_type then
    ...
    --- Distance
    local distance_label = distance_gui["param_distance.label"]
    local distance_interface = distance_gui["param_distance.interface"]
    ...
    --- Direction
    local direction_interface = direction_gui["param_direction.interface"]
    ---@type table<number, string|LocalisedString>
    local direction_items = {
      RuntimeParamType.train.arrival_front_dir,
      RuntimeParamType.train.arrival_rear_dir,
    }
    ...
    for i, t in pairs(direction_items) do
      ...
      direction_items[i] = {"rsad-actions." .. t}
    end
...
for name, updator in pairs(builtin_action_updators) do
  local class = {update = updator}
  glib.register_class(name, class)
  g_builder_classes[name] = class
end
---
...
function procedure_builder.build_action_node(action)
  return build_node(action.custom_name, action.internal_type, builtin_action_param_guis[action.internal_type])
end

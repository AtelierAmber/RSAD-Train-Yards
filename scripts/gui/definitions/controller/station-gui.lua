...
function handle_item(e)
  ...
  if element.elem_value then
    --[[@type SignalID]]
    local signal = {}
    if element.elem_type == "fluid" then
      signal.type = "fluid"
    ...
    update_rsad_station_name(entity, control,
      bit32.extract(control.circuit_condition.constant, STATION_TYPE_ID, STATION_TYPE_ID_WIDTH))
...
local function type_gui_drop_down(entity, selected_index)
  return {
    type = "flow",
    name = "type_gui",
    direction = "horizontal",
...
local function shunting_switch(entity, reversed)
  return {
    type = "flow",
    name = "hflow_switch",
    direction = "horizontal",
...
local function subtype_drop_down(entity, subinfo)
  return {
    type = "flow",
    name = "vflow_subtype",
    direction = "vertical",
    ...
    children = {
      {
        ...
        children = {
          ...
          },
          {
            ...
            tags = { id = entity.unit_number },
            selected_index = ((subinfo >= rsad_shunting_stage.sort_imports and subinfo < rsad_shunting_stage.return_to_depot) and subinfo) or
                0,
            items = {
...
---@param entity LuaEntity
local function train_limit_bar(entity)
  return {
    type = "flow",
...
---@param entity LuaEntity
---@param subinfo integer
local function cargo_limit_bar(entity, subinfo)
  return {
    type = "flow",
...
local function network_selection(entity, network)
  return {
    type = "flow",
    name = "hflow_network",
    direction = "horizontal",
...
---@param item SignalID?
---@param switch string
local function item_selection(entity, item, switch)
  return {
    type = "flow",
    name = "hflow_item",
    direction = "horizontal",
...
---@return GuiElemDef
function station_gui(entity, player, selected_index, network, item, reversed, subinfo, item_switch)
  return {
    type = "frame",
    direction = "vertical",
...
function open_station_gui(rootgui, entity, player)
  ...
  local _, mainscreen = flib_gui.add(rootgui,
    { station_gui(entity, player, selected_type, network, item, reversed, subinfo, item_switch) })
  mainscreen.frame.vflow_main.preview_frame.preview.entity = entity
  mainscreen.titlebar.drag_target = mainscreen
  mainscreen.titlebar.titlebar_label.caption = { "", "RSAD ", { "rsad-gui.station-types.station-" .. (selected_type - 1) },
    " Station" }
